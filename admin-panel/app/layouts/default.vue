<template>
  <div class="min-h-screen bg-brand-bg text-brand-cream font-sans flex flex-col lg:flex-row">
    
    <!-- Mobile Header & Drawer Trigger -->
    <header class="lg:hidden flex items-center justify-between p-4 border-b border-brand-gold/10 bg-brand-cocoa-dark">
      ...
      <button @click="isDrawerOpen = true" class="text-brand-gold p-2 min-h-[44px] min-w-[44px] flex items-center justify-center">
    ...
    <USlideover v-model="isDrawerOpen" side="left" class="lg:hidden">
      <div class="p-6 bg-brand-cocoa-dark h-full border-r border-brand-gold/10 flex flex-col" style="padding-bottom: env(safe-area-inset-bottom);">
        <div class="flex items-center justify-between mb-8">
          ...
          <button @click="isDrawerOpen = false" class="text-brand-cream/50 min-h-[44px] min-w-[44px] flex items-center justify-center">
        ...
        <button @click="logout" class="mt-auto flex items-center space-x-3 px-4 py-3 text-brand-cream/50 hover:text-brand-cream transition-colors uppercase text-[10px] tracking-widest font-bold border-t border-brand-gold/10 pt-6 min-h-[44px]">
    ...
    <!-- Desktop Sidebar -->
    <aside class="hidden lg:flex flex-col w-64 border-r border-brand-gold/10 bg-brand-cocoa-dark h-screen sticky top-0 shrink-0">
      ...
      <div class="p-6 border-t border-brand-gold/5 bg-brand-bg/20">
        <button @click="logout" class="w-full flex justify-center items-center space-x-2 text-[10px] uppercase tracking-widest font-bold text-brand-cream/40 hover:text-brand-gold transition-colors duration-300 min-h-[44px]">
          <UIcon name="i-heroicons-arrow-right-on-rectangle" class="w-4 h-4" />
          <span>Secure Sign Out</span>
        </button>
      </div>
    ...
    <!-- Main Content Area -->
    <main class="grow p-6 lg:p-12 overflow-x-hidden">
      <slot />
    </main>
  </div>
</template>
